...
{
    tm tmp;
    sscanf(xmlStr.c_str(), "%04d-%02d-%02dT%02d:%02d:%02dZ", 
           &tmp.tm_year,
           &tmp.tm_mon,
           &tmp.tm_mday,
           &tmp.tm_hour,
...
time_t
DateTime::StrtoTimeT(std::string& xmlStr)
{
    tm tmp;
    sscanf(xmlStr.c_str(), "%04d-%02d-%02dT%02d:%02d:%02dZ", 
           &tmp.tm_year,
...
std::string
DateTime::TimeTtoXmlStr(time_t timet)
{
    std::string        retVal;
    std::ostringstream oss;
    struct tm tmbuf;
    ...
    if ((tmInfo->tm_mon + 1) <= 9) oss << "0";
    oss << tmInfo->tm_mon + 1 << "-";
    if (tmInfo->tm_mday <= 9) oss << "0";
    oss << tmInfo->tm_mday << "T";
    if (tmInfo->tm_hour <= 9) oss << "0";
    oss << tmInfo->tm_hour << ":";
    if (tmInfo->tm_min <= 9) oss << "0";
    oss << tmInfo->tm_min << ":";
    if (tmInfo->tm_sec <= 9) oss << "0";
    oss << tmInfo->tm_sec << "Z";
    retVal = oss.str();
    return retVal;
}    
...
std::string
DateTime::StrtoTimeTStr(std::string& xmlStr)
{
    tm tmp;
    std::ostringstream oss;
    sscanf(xmlStr.c_str(), "%04d-%02d-%02dT%02d:%02d:%02dZ", 
...
time_t
DateTime::addDuration(std::string xmlDuration)
{
...
void
DateTime::addDurationDateTime(std::string xmlDuration)
{
...
int
DateTime::maxDayInMonthFor(int yr, int month)
{
    ...
    if (tmpMonth == MON_JAN || tmpMonth == MON_MAR || tmpMonth == MON_MAY
        ...
        if (tmpMonth == MON_APR || tmpMonth == MON_JUN|| tmpMonth == MON_SEP
            ...
            if (tmpMonth == MON_FEB && 
                ((0 == tmpYear%400) || ((0 != tmpYear%100) && 0 == tmpYear%4))){
                return 29;
            }
            else
                return 28;
...
void
DateTime::parseDuration(std::string durStr, struct tm& tmT)
{
...
bool
DateTime::isValidXMLDatetime(const std::string& str)
{
  ...
  tm tmp;
  int ret = sscanf(str.c_str(), "%04d-%02d-%02dT%02d:%02d:%02dZ",
		   &tmp.tm_year,
		   &tmp.tm_mon,
		   &tmp.tm_mday,
		   &tmp.tm_hour,
  ...
  if(ret != 6) return false;//should have 6 match

  if( tmp.tm_mon > 12 || tmp.tm_mon <= 0 )  return false;
  if(tmp.tm_mday > 31 || tmp.tm_mday <= 0)  return false;
  if(tmp.tm_hour > 24 || tmp.tm_hour < 0)  return false;
  if( tmp.tm_min > 60 || tmp.tm_min < 0 )  return false;
  if( tmp.tm_sec > 60 || tmp.tm_sec < 0 )  return false;

 ...
 */
unsigned int
Threadpool::addThreads(unsigned int threads)
{
    unsigned int i;
    pthread_t pThread;
    ...
    pthread_attr_setdetachstate(&pThreadAttrs,
    ...
    for (i=0; i < threads; i++) {
        int stat = pthread_create(&pThread,
                                  &pThreadAttrs,
 ...
 */
unsigned int
Threadpool::killThreads(unsigned int threads)
 ...
 */
unsigned int
Threadpool::genKillThreadEvents(unsigned int toKill)
{
 ...
 */
void
Threadpool::schedule(Stage* stageP)
{
    ...
    if (!wasEmpty || this != getThreadPoolPtr()) {
 ...
 */
void*
Threadpool::runThread(void* poolPtr)
{
    ...
    pid_t threadid;
    threadid = gettid();
    LOG_INFO("threadid = %d, threadname = %s\n", threadid, poolP->getName().c_str());
...
void
Threadpool::finalizeThreadPool()
{
    pthread_key_delete(poolPtrKey);
}

void
Threadpool::setThreadPoolPtr(const Threadpool* thdPool)
{

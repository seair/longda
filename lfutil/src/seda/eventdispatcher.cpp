...
// Include Files
#include "seda/eventdispatcher.h"
 ...
 */
void
EventDispatcher::handleEvent(StageEvent* event)
{
    LOG_TRACE( "enter\n");
...
EventDispatcher::cleanup()
{
    ...
    // for each hash chain...
    for (EventHash::iterator i = eventStore.begin();
         i != eventStore.end();
         i++) {
...
bool
EventDispatcher::wakeupEvent(std::string hashkey)
{
    bool                sent = false;
    ...
    if (i != eventStore.end()) {
        ...
        status_t stat = dispatchEvent(targetEv.first,
                                      targetEv.second,
                                      hashkey);
        if (stat == SEND_EVENT) {

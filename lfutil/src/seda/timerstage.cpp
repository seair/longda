...
void realtimeToMonotonic(const struct timeval* time_RT, struct timeval* time_Mono)
{
...
void
TimerToken::set(const struct timeval& t, u64_t n)
{
...
TimerToken::getTime() const
{
    return time;
}

u64_t
TimerToken::getNonce() const
{
    return nonce;
}

bool
TimerToken::operator<(const TimerToken& other) const
{
...
const struct timeval&
TimerRegisterEvent::getTime()
{
    return timer_when;
}

StageEvent*
TimerRegisterEvent::getCallbackEvent()
{
    return timer_cb;
}

StageEvent*
TimerRegisterEvent::adoptCallbackEvent()
{
...
void
TimerRegisterEvent::setCancelToken(const TimerToken& t)
{
    token = t;
    return;
}

std::auto_ptr<const TimerToken>
TimerRegisterEvent::getCancelToken()
{
    const TimerToken* token_cp = new TimerToken(token);
    std::auto_ptr<const TimerToken> token_ptr(token_cp);
    return token_ptr;
}
...
const TimerToken&
TimerCancelEvent::getToken()
{
    return token;
}

void
TimerCancelEvent::setSuccess(bool s)
{
    cancelled = s;
    return;
}
...
TimerStage::TimerStage(const char* tag) :
    Stage(tag),
    timer_queue(&TimerStage::timerTokenLessThan),
    shutdown(false),
    num_events(0),
    timer_thread_id(0)
{
...
{
    for (timer_queue_t::iterator i = timer_queue.begin();
         i != timer_queue.end(); ++i)
    {
        delete i->second;
    }
...
Stage* 
TimerStage::makeStage(const std::string& tag)
{
...
u32_t
TimerStage::getNumEvents()
{
    return num_events;
}

void
TimerStage::disconnectPrepare()
{
...
void
TimerStage::handleEvent(StageEvent* event)
{
    ...
    TimerRegisterEvent* register_ev =
        dynamic_cast<TimerRegisterEvent*>(event);
    if (register_ev != NULL)
    {
    ...
    TimerCancelEvent* cancel_ev =
        dynamic_cast<TimerCancelEvent*>(event);
    if (cancel_ev != NULL)
    {
...
void
TimerStage::callbackEvent(StageEvent* e, CallbackContext* ctx)
{
    return;
}

void
TimerStage::registerTimer(TimerRegisterEvent& reg_ev)
{
    ...
    ASSERT(result.second, "Internal error--"
...
void
TimerStage::cancelTimer(TimerCancelEvent& cancel_ev)
{
    pthread_mutex_lock(&timer_mutex);
    bool success = false;
...
void*
TimerStage::startTimerThread(void* arg)
{
...
TimerStage::checkTimer()
{
    ...
    while (true)
    {
        ...
        if (! done_events.empty())
        {
            ...
            for (std::list<StageEvent*>::iterator i =
                     done_events.begin();
                 i != done_events.end(); ++i)
            {
...
bool
TimerStage::timerTokenLessThan(const TimerToken& tt1, const TimerToken& tt2)
{
    return (tt1 < tt2);
}

...
MD5Update(MD5_CTX        *context, const unsigned char  *input, unsigned int    inputLen)
{
    ...
    if (inputLen >= partLen) {
        MD5_memcpy
            ((POINTER) & context->buffer[index], (const POINTER) input, partLen);
    ...
    MD5_memcpy
        ((POINTER) & context->buffer[index], (const POINTER) & input[i],
         inputLen - i);
}
 ...
 * Digests a string
 */
int MD5String(const char *string,unsigned char digest[16])
{
    MD5_CTX context;
...
int MD5Buffer(const char *buffer, unsigned int len, unsigned char digest[16])
{
    MD5_CTX context;
